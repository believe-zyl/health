...
bool max30102_write_reg(uint8_t uch_addr, uint8_t uch_data)
{
    ...
    IIC_Send_Byte(max30102_WRITE_ADDR);	/* 此处是写指令 */
...
bool max30102_read_reg(uint8_t uch_addr, uint8_t *puch_data)
{
    ...
    IIC_Send_Byte(max30102_READ_ADDR);	/* 此处是读指令 */
...
bool max30102_read_fifo(uint32_t *pun_red_led, uint32_t *pun_ir_led)
{
    ...
    *pun_ir_led = 0;
    *pun_red_led = 0;
    max30102_read_reg(REG_INTR_STATUS_1, &uch_temp);    //通过读取寄存器来清除中断状态/相当于中断标志初始化
    max30102_read_reg(REG_INTR_STATUS_2, &uch_temp);    //通过读取寄存器来清除中断状态/相当于中断标志初始化
