...
void maxim_heart_rate_and_oxygen_saturation(uint32_t *pun_ir_buffer, int32_t n_ir_buffer_length, 
	uint32_t *pun_red_buffer, int32_t *pn_spo2, int8_t *pch_spo2_valid, int32_t *pn_heart_rate, int8_t *pch_hr_valid)
{
    uint32_t un_ir_mean, un_only_once;
    int32_t k, n_i_ratio_count;
    int32_t i, m, n_exact_ir_valley_locs_count, n_middle_idx;
    ...
    for(k=0; k<n_npks; k++)
        ir_valley_locs[k] = an_ir_valley_locs[k] + HAMMING_SIZE/2; 
    ...
    for(k=0; k<n_exact_ir_valley_locs_count; k++) {
        if(an_exact_ir_valley_locs[k] > 500) {
